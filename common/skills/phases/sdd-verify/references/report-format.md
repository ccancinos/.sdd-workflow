...
Write this report to `openspec/changes/{change-name}/verify-report.md`.
...
Counts come from the actual specs retrieved from `openspec/changes/{change-name}/specs/`. Never copy example totals. A `FAIL` verdict with blockers, critical findings, or a non-zero command exit is valid and MUST still be persisted, but such a change is not archive-ready. Human prose in the report never overrides the recorded test/build exit codes.
