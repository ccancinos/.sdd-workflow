...
> **This module is loaded ONLY when Strict TDD Mode is enabled (`strict_tdd: true` in `openspec/config.yaml`) AND a test runner is available.**
> If you are reading this, the verify phase already verified both conditions. Follow every instruction.

## TDD Verification Philosophy

When Strict TDD Mode is active, verification goes beyond "does the code work?" to "was the code built correctly?" — meaning: was TDD actually followed? The apply phase reports TDD evidence (its **TDD Cycle Evidence** table); your job is to validate that evidence against reality.

## Step 5a: TDD Compliance Check (includes Assertion Quality Audit)

Read the apply phase's **TDD Cycle Evidence** table (from its return summary) and verify that TDD was actually followed:

```
Read the TDD Cycle Evidence table:
├── FOR EACH task row:
│   ├── RED column:
│   │   ├── Must say "✅ Written"
...
```
IF coverage tool available (from config testing capabilities):
├── Run: {test_command} --coverage (or equivalent)
├── Parse the coverage report
├── Filter to ONLY files created or modified in this change
│   (get file list from the apply phase's "Files Changed" table)
├── Report per-file:
│   ├── File path
│   ├── Line coverage %
...
```
Read quality tools from config testing capabilities:
...
|-------|--------|---------|
| TDD Evidence reported | ✅ / ❌ | {Found in apply summary / Missing} |
...
- ALWAYS check the TDD Cycle Evidence table from the apply phase — it's the primary artifact
...
- If the apply summary has no TDD evidence table, flag as CRITICAL — the protocol was not followed
