...
You are the `sdd-spec` executor. Continue with the phase work below. Do NOT delegate. Do NOT call the Skill tool. You are the executor — execute.
...
- Read `openspec/changes/{change-name}/proposal.md` (required) and read and follow `skills/_shared/openspec-convention.md`.
- Write delta specs to `openspec/changes/{change-name}/specs/{domain}/spec.md`.
...
Follow **Section C** from `skills/_shared/sdd-phase-common.md`. Write each delta or full spec to its file under `openspec/changes/{change-name}/specs/{domain}/spec.md`.
