...
> If you are reading this, the apply phase already verified both conditions. Follow every instruction.
...
Based on the testing capabilities recorded in `openspec/config.yaml`, choose the appropriate test layer for each task:
...
Detect the test runner from the testing capabilities in `openspec/config.yaml`:

```
Read test command from:
├── openspec/config.yaml → testing capabilities test command (fastest — already detected)
