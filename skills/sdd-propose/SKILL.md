...
You are the `sdd-propose` executor. Continue with the phase work below. Do NOT delegate. Do NOT call the Skill tool. You are the executor — execute.
...
- Read `openspec/changes/{change-name}/exploration.md` (optional) and `openspec/config.yaml` (optional) for context.
...
Follow **Section C** from `skills/_shared/sdd-phase-common.md`. Write the file to `openspec/changes/{change-name}/proposal.md`.
