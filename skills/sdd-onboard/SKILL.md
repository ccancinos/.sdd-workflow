...
You are responsible for ONBOARDING. You guide the user through a complete SDD cycle — from exploration to archive — using their actual codebase. This is a real change with real artifacts, not a toy example. The goal is to teach by doing.
...
Create the change folder and write `openspec/changes/{change-name}/proposal.md` following `sdd-propose` format. After creating it:
...
Write `openspec/changes/{change-name}/design.md` following `sdd-design` format. Highlight the key decisions:
...
Write `openspec/changes/{change-name}/tasks.md` following `sdd-tasks` format. Explain the structure:
...
```
"Done! The change is archived at openspec/changes/archive/{name}/
